...
async def gstats_global(client, message: Message, _):
    ...
    title = title.title()
    final = f"» الاكثر تشغيلا في بوت {MUSIC_BOT_NAME}\n\n**الاسم:** {title}\n\nتم تشغيله** {co} **مرة."
    ...
    def get_stats():
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    msg += f"🍒 [ᴛᴇʟᴇɢʀᴀᴍ ᴍᴇᴅɪᴀ](https://t.me/YOKA_SUPPORT) ** تم تشغيله {count} مرة**\n\n"
                else:
                    msg += f"📌 [{title}](https://www.youtube.com/watch?v={items}) ** تم تشغيله {count} مرة**\n\n"
    ...
    if what in ["Users", "Chats"]:
        for items, count in list_arranged.items():
            ...
            try:
                ...
                continue
            limit += 1
            msg += f"💖 `{extract}` تم تشغيله {count} مرة في البوت.\n\n"
        temp = (
...
async def overall_stats(client, CallbackQuery, _):
    if CallbackQuery.from_user.id not in SUDOERS:
        return await CallbackQuery.answer(
            "خاص بالمطورين فقط.", show_alert=True
        )
    callback_data = CallbackQuery.data.strip()
    what = callback_data.split(None, 1)[1]
    if what != "s":
