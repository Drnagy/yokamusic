...
async def auth(client, message: Message, _):
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text("**» قم بالرد علي المستخدم لرفعه ادمن./n- او قم بكتابة الامر مع يوزر المستخدم...**")
        ...
        count = len(_check)
        if int(count) == 20:
            return await message.reply_text("**» يمـڪنڪ رفـع 20 ادمن فقـط.**")
        if token not in _check:
            ...
            return await message.reply_text("**» تم رفعـه ادمن بنجـاح ✓.**")
        else:
            await message.reply_text("**» المستخـدم بالفعـل فـي قائمـة الادمنيـة✓.**")
        return
    from_user_id = message.from_user.id
    ...
    for smex in _check:
        count += 1
    if int(count) == 20:
        return await message.reply_text("**» يمـڪنـڪ رفـع 20 ادمن فقـط.**")
    if token not in _check:
        assis = {
...
async def unauthusers(client, message: Message, _):
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text("**ـ قم بالرد علي المستخدم لرفعه ادمن.
ـ او اكتب الامر مع اليوزر او الايدي الخاص بالمستخدم..**")
        user = message.text.split(None, 1)[1]
        if "@" in user:
            user = user.replace("@", "")
        user = await app.get_users(user)
        ...
        if deleted:
            ...
            return await message.reply_text("**» تم تنزيله من الادمنيـة بنجـاح ✓**")
        else:
            return await message.reply_text("**» المستخدم ليس ادمن بالاصل!.**")
    user_id = message.reply_to_message.from_user.id
    token = await int_to_alpha(user_id)
    deleted = await delete_authuser(message.chat.id, token)
    get = adminlist.get(message.chat.id)
